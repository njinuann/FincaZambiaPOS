        ...
        implements ChannelAdaptorMBean, Channel, Loggeable
{
    ...
    public void startService()
    {
        ...
        {
            getLog().warn("<message>Error starting TXNListener</message>", e);
        }
    }
            ...
            throws ConfigurationException
    {
        ...
        ISOChannel lChannel = (ISOChannel) f.newInstance(channelName);
        ISOPackager packager = null;
        if (packagerName != null)
        {
            packager = (ISOPackager) f.newInstance(packagerName);
            lChannel.setPackager(packager);
            f.setConfiguration(packager, e);
        }
        QFactory.invoke(lChannel, "setHeader", e.getAttributeValue("header"));
        f.setLogger(lChannel, e);
        f.setConfiguration(lChannel, e);

        if (lChannel instanceof FilteredChannel)
        {
            addFilters((FilteredChannel) lChannel, e, f);
        }
        if (getName() != null)
        {
            lChannel.setName(getName());
        }
        return lChannel;
    }
