 ...
 * @version $Revision: 2993 $ $Date: 2010-10-13 16:34:48 -0200 (Wed, 13 Oct
 * 2010) $
 */
public class Q2 implements FileFilter, Runnable
{

    public static final String Q2_VERSION = "1.7.0";
    public static final String Q2_REVISION = "1.7.0 2";
    public static final String Q2_BUILD_DATE = "19-JUN-2011 12:42 PM";
    public static final String DEFAULT_DEPLOY_DIR = "conf";
    public static final String JMX_NAME = "Q2";
    public static String LOGGER_NAME = "Q2";
    public static String REALM = "Q2.system";
    ...
    private File confDir, libDir, logDir;
    private Map dirMap;
    ...
    public Q2(String[] args)
    {
        ...
        parseCmdLine(args);
        libDir = new File(confDir, "lib");
        dirMap = new TreeMap();
        confDir.mkdirs();
        mainClassLoader = Thread.currentThread().getContextClassLoader();
    }

    public Q2(String confFolder, String logFolder, String logger, String realm)
    {
        counter++;
        loggerName = logger;
        ...
        confDir = new File(confFolder);
        logDir = new File(logFolder);
        startTime = System.currentTimeMillis();
        instanceId = UUID.randomUUID();
        parseCmdLine(args);
        libDir = new File(confFolder, "lib");
        dirMap = new TreeMap();
        confDir.mkdirs();
        logDir.mkdirs();
        mainClassLoader = Thread.currentThread().getContextClassLoader();
    ...
    public void run()
    {
        ...
        try
        {
            ...
            {
                //log.error(e); Okey to ignore for shutdown
            }
            if (decorator != null)
            {
    ...
    {
        boolean rc = false;
        File file[] = confDir.listFiles(this);
        // Arrays.sort (file); --apr not required - we use TreeMap
        if (file == null)
        {
            // Shutting down might be best, how to trigger from within?
            throw new Error("Deploy directory \"" + confDir.getAbsolutePath() + "\" is not available");
        }
        else
        {
            for (File f : file)
    ...
    private boolean deploy(File f)
    {
        ...
        catch (InstanceAlreadyExistsException e)
        {
             ...
             *
             */
            //tidyFileAway(f, DUPLICATE_EXTENSION); Peter......TPB Project
        ...
        {
            getLog().warn("deploy", e);
            //tidyFileAway(f, ERROR_EXTENSION); Peter......TPB Project
    ...
    {
        logDir.mkdirs();
        File loggerConfig = new File(confDir, LOGGER_CONFIG);
        ...
        getLog().info("Q2 started, deployDir=" + confDir.getAbsolutePath());
    }
            ...
            throws ISOException, IOException, GeneralSecurityException
    {
        ...
        Document doc = new Document();
        doc.setRootElement(e);
        File qbean = new File(confDir, fileName);
        if (isTransient)
        {
