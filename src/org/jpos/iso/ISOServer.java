 ...
 */
public class ISOServer extends Observable implements LogSource, Runnable, Observer, ISOServerMBean, Configurable, Loggeable
{
    int port;
    ...
    protected class Session implements Runnable, LogSource
    {
        ...
        public void run()
        {
            ...
            try
            {
                for (;;)
                {
                    try
                    {
                        ...
                        while (iter.hasNext())
                        {
                            if (((ISORequestListener) iter.next()).process(channel, m))
                            {
                                break;
                            }
                        }
            ...
            try
            {
                channel.disconnect();
                setChanged();
                notifyObservers(ISOServer.this);
            }
    ...
    public void run()
    {
        ...
        while (!shutdown)
        {
            try
            {
                serverSocket = socketFactory != null ? socketFactory.createServerSocket(port) : (new ServerSocket(port, backlog, bindAddr));
                Logger.log(new LogEvent(this, "iso-server", "Host started and listening"));
    ...
    public String getCountersAsString()
    {
        ...
        sb.append("<detail>connected=");
        sb.append(Integer.toString(counters[0]));
        sb.append(", rx=");
        sb.append(Integer.toString(counters[0]));
        sb.append(", tx=");
        sb.append(Integer.toString(counters[1]));
        sb.append(", last=");
        sb.append(lastTxn).append("</detail>");
        if (lastTxn > 0)
        {
            sb.append(", idle=");
