 ...
 */
/**
 * ISOChannel is an abstract class that provides functionality that allows the
 * transmision and reception of ISO 8583 Messages over a TCP/IP session.
 * <p>
 * It is not thread-safe, ISOMUX takes care of the synchronization details
 * <p>
 * ISOChannel is Observable in order to suport GUI components such as
 * ISOChannelPanel.
 * <br>
 * It now support the new Logger architecture so we will probably setup
 * ISOChannelPanel to be a LogListener insteado of being an Observer in future
 * releases.
 *
 * @author Alejandro P. Revilla
 * @author Bharavi Gade
 * @version $Revision: 2999 $ $Date: 2010-10-14 21:01:39 -0200 (Thu, 14 Oct
 * 2010) $
 * @see ISOMsg
 * @see MUX
 * @see ISOException
...
//            System.out.print(strFileContents);
    ...
    protected Socket newSocket(String host, int port) throws IOException
    {
        try
        {
            ...
            {
                connectTimeout = 5000;//Peter.... Connection Timeout = 5 seconds
                if (connectTimeout > 0)
                {
                    Socket s = new Socket();
                    s.connect(
    ...
    /**
     * @param b - new Usable state (used by ISOMUX internals to flag as unusable
     * in order to force a reconnection)
     */
...
//            //njinu dump
//            m.dump(System.out, "");
//            System.out.println(MTI + " this is the response mti outgoing");
//            System.out.println(ISOUtil.hexString(b));
//            String s = new String(b);
//            System.out.println("show bytes " + s);
//
//            m.dump(System.out, "");
...
//            //njinu
//            System.out.println(" this is the response mti outgoing");
//            System.out.println(ISOUtil.hexString(b));
//            String s = new String(b);
//            System.out.println("show bytes " + s);
            cnt[TX]++;
    ...
    {
        byte[] localHeader = new byte[hLen];
        serverIn.readFully(localHeader, 0, hLen);
        return localHeader;
    }
    ...
    public ISOMsg receive() throws IOException, ISOException
    {
        ...
        String MTI = "";
        byte[] lHeader = null;
        boolean error = false, isDisconnect = false;
        ...
        try
        {
            ...
            m.setPackager(getDynamicPackager(lHeader, b));
            m.setHeader(getDynamicHeader(lHeader));

            if (b.length > 0 && !shouldIgnore(lHeader))  // Ignore NULL messages
            {
                unpack(m, b);
            }

//
//            System.out.println("header " + ISOUtil.hexString(lHeader));
            //   System.out.println(ISOUtil.hexString(b));
            m.setDirection(ISOMsg.INCOMING);
            evt.addMessage(m);
            m = applyIncomingFilters(m, lHeader, b, evt);
            m.setDirection(ISOMsg.INCOMING);
            MTI = m.getMTI();
//            //Njinu
//            System.out.println("--- incoming data ---");
//            System.out.pri Systentln(MTI + " this is the response mti");
//            System.out.println(ISOUtil.hexString(b));
//            String s = new String(b);
//            System.out.println("show bytes " + s);
            cnt[RX]++;
        ...
        catch (ISOException e)
        {
            ...
            if (lHeader != null)
            {
                ...
                evt.addMessage(ISOUtil.hexdump(lHeader));
            }
            if (b != null)
            {
                evt.addMessage("--- data ---");
                ...
                //Njinu
//                System.out.println("--- incoming data2 ---");
//                System.out.println(b.length + " this is the messahe length");
//                System.out.println(ISOUtil.hexString(b));
//                String s = new String(b);
//                System.out.println("show bytes " + s);
            }
        ...
        {
            if (error == true && isDisconnect == false)
            {
                Logger.log(evt);
            }
        }
    ...
    /**
     * disconnects the TCP/IP session. The instance is ready for a reconnection.
     * There is no need to create a new ISOChannel<br>
     *
     * @exception IOException
     */
