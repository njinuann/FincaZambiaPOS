...
public class ISOChannelPanel extends JPanel implements Observer
{
    ...
    public final void setLog(ListModel listModel)
    {
        log = (DefaultListModel) listModel;
    }
    ...
    public void update(Observable o, Object arg)
    {
        if (arg != null && arg instanceof ISOMsg)
        {
            ...
            try
            {
                ...
                if (m.getMTI().startsWith("01") || m.getMTI().startsWith("02") || m.getMTI().startsWith("04") || m.getMTI().startsWith("09") || m.getMTI().startsWith("12") || m.getMTI().startsWith("14") || m.getMTI().startsWith("19") || m.getMTI().startsWith("9"))
                {
                    log.addElement(m);
                }
        ...
        if (o instanceof BaseChannel)
        {
            ...
            try
            {
                meter.setPositiveCounter(
                        ISOUtil.zeropad(Integer.toString(cnt[ISOChannel.TX % 1000000000]), 3));
                meter.setNegativeCounter(
                        ISOUtil.zeropad(Integer.toString(cnt[ISOChannel.RX] % 1000000000), 3));
            }
        ...
        else if (o instanceof ISOServer)
        {
            ...
            final Runnable updateIt = new Runnable()
            {
                public void run()
                {
                    ...
                    {
                        meter.setPositiveCounter(
                                ISOUtil.zeropad(Integer.toString(active), 3));
                    }
                    catch (ISOException e)
                    {
                    }
