...
public class ISOMsg extends ISOComponent implements Cloneable, Loggeable, Externalizable
{
     ...
     */
    public final void setFieldNumber(int fieldNumber)
    {
    ...
    public String getAcctType1()
    {
        if (hasField(3))
        {
            String field3 = getString(3);
            return (field3.length() >= 4) ? field3.substring(2, 4) : "01";
        }
        return "01";
    }
    ...
    public String getAcctType2()
    {
        ...
        {
            String field3 = getString(3);
            return (field3.length() >= 6) ? field3.substring(4, 6) : "01";
        }
        return "01";
    }
    ...
    public String getProcessingCode()
    {
        ...
        {
            String field3 = getString(3);
            return (field3.length() >= 2) ? field3.substring(0, 2) : "";
        }
        return null;
    }
    ...
    public String getAccountType1()
    {
        ...
        {
            String field3 = getString(3);
            return (field3.length() >= 2) ? field3.substring(2, 4) : "";
        }
        return null;
    }
    ...
    public String getAccountType2()
    {
        ...
        {
            String field3 = getString(3);
            return (field3.length() >= 2) ? field3.substring(4, 6) : "";
        }
        return null;
    }
     ...
     * getComposite(). A <b>Composite component</b> returns itself and a Leaf
     * returns null.
     *
     * @return ISOComponent
     */
     ...
     * be easily parsed.
     * <br>
     * Each component is responsible for its own dump function, ISOMsg just
     * calls dump on every valid field.
     *
    ...
    /**
     * Don't call setValue on an ISOMsg. You'll sure get an ISOException. It's
     * intended to be used on Leafs
     *
     * @see ISOField
     * @see ISOException
     */
     ...
     * please note that received fields take precedence over existing ones
     * (simplifying card agent message creation and template handling)
     *
    ...
    public String toString()
    {
        ...
        if (hasField(102))
        {
            if (!"5036979999999999997".equals(getString(102).trim()))
            {
                s.append(" [ ");
                s.append(getString(102).trim());
                s.append(" ]");
            }
            else if (hasField(103))
            {
                s.append(" [ ");
                s.append(getString(103).trim());
                s.append(" ]");
            }
        }
        else if (hasField(103))
     ...
     */
    public final void setMTI(String mti) throws ISOException
    {
        if (isInner())
        {
    ...
    {
        if (!isInner() && hasField(0))
        {
            String field0 = (String) getString(0);
            return (field0.length() > 4) ? field0.substring(field0.length() - 4) : field0;
        }
        return "0000";
    }
    ...
    /**
     * sets an appropriate response MTI. i.e. 0100 becomes 0110<br>
    ...
    /**
     * Let this ISOMsg object hold a weak reference to an ISOSource (usually
     * used to carry a reference to the incoming ISOChannel)
     *
    ...
    public String getAccessCode()
    {
        if (hasField(2))
        {
            if (getString(2).equals(getString(102)))
            {
                return getString(35);
            }
            return getString(2);
    ...
    public BigDecimal getTxnAmount()
    {
        if (hasField(4))
        {
            try
            {
                return new BigDecimal(getString(4)).divide(new BigDecimal(100)).setScale(2);
            }
            catch (Exception e)
            {
    ...
    public BigDecimal getChargeAmount()
    {
        if (hasField(28))
        {
            try
            {
                return new BigDecimal(getString(28).substring(1)).divide(new BigDecimal(100)).setScale(2);
            }
            catch (Exception e)
            {
                return BigDecimal.ZERO;
    ...
    public BigDecimal getReversedTxnAmount()
    {
        if (hasField(6))
        {
            try
            {
                return new BigDecimal((Double.parseDouble(getString(6)) / 100)).setScale(2, RoundingMode.DOWN);
            }
            catch (Exception e)
            {
                return BigDecimal.ZERO;
...
//    {
//        if (hasField(11) && hasField(7))
//        {
//            return getString(11).trim() + getString(7).trim();
//        }
//        return "0";
//    } commented out for finca zambia reference number to have field 37 thus combination of F7&F37
    public String getTxnReference()
    {
        if (hasField(7) && hasField(37))
        {
            return getString(7).trim() + getString(37).trim();
        }
        return "0";
    }
    ...
    public void setResponseCode(String respCode) throws ISOException
    {
        set(39, respCode);
    }
    ...
    public String getCardAcceptorName()
    {
        if (hasField(43))
        {
            return getString(43).trim();
        }
        return getTerminalID();
    }

    public String getStringNotNull(int fieldNo)
    {
        String val = getString(fieldNo);
        return ((val == null) ? "" : val);
    }
...
//        if (hasField(90))
//        {
//            return getString(90).substring(4, 20).trim();
//        }
//        return getTxnReference();
//    }commented out for finca zambia reversal reference number to have  combination of F7&F37 in original data elements
    public String getOriginalTxnReference()
    {
        if (hasField(90))
        {
            return getString(90).substring(10, 32).trim();
        }
        return getTxnReference();
    }
    ...
    public String getAccountNumber1()
    {
        if (hasField(102))
        {
            return getString(102).trim();
        }
        return null;
    }

    public void setAccountNumber1(String acctNo)
    {
        try
        {
            if (acctNo != null)
    ...
    public String getXapiCode()
    {
        if (hasField(121))
        {
            return getString(121).trim();
        }
        return null;
    }

    public void setXapiCode(String xapiCode) throws ISOException
    {
        set(121, xapiCode);
    }

    public String getXapiResponse()
    {
        if (hasField(122))
        {
            return getString(122).trim();
        }
        return null;
    }

    public void setXapiResponse(String xapiMessage) throws ISOException
    {
        set(122, xapiMessage);
    }
}
