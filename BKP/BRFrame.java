 ...
 * @author Pecherk
 */
public class BRFrame extends javax.swing.JFrame
{

    ListModel txnLog = null;
    ArrayList<EITxn> txnList = new ArrayList();
    private final EILogin eiLogin = new EILogin(new TDClient());
    private final BSPanel settingsPanel = new BSPanel(PHController.Module);
    ...
    private void initComponents()
    {
        ...
        setTitle("PHilae POS Bridge ~ Neptune Software");
        ...
        procCodeMenuItem.setAccelerator(javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_F, java.awt.event.InputEvent.ALT_MASK | java.awt.event.InputEvent.SHIFT_MASK));
        ...
        suspendServiceCheckBoxMenuItem.setSelected("Y".equalsIgnoreCase(PHilae.PHController.SuspendPOS));
        suspendServiceCheckBoxMenuItem.setText("Y".equalsIgnoreCase(PHilae.PHController.SuspendPOS) ? "Resume Service" : "Suspend Service");
...
//        }
    ...
    {//GEN-HEADEREND:event_agencySettingsMenuItemActionPerformed
        // TODO add your handling code here:
        if (eiLogin.isUserAdmin(capitalize(PHController.Module) + " Settings"))
        {
    ...
    {//GEN-HEADEREND:event_chargesMenuItemActionPerformed
        // TODO add your handling code here:
        if (eiLogin.isUserAdmin(capitalize(PHController.Module) + " Charges"))
        {
            showChargesDialog();
        }
    ...
    {//GEN-HEADEREND:event_extractDataFilesMenuItemActionPerformed
        // TODO add your handling code here:
        if (eiLogin.isUserAdmin(capitalize(PHController.Module) + " Data Files"))
        {
    ...
    {//GEN-HEADEREND:event_eiTerminalMenuItemActionPerformed
        // TODO add your handling code here:
        if (eiLogin.isUserAdmin(capitalize(PHController.Module) + " Terminals"))
        {
    ...
    {//GEN-HEADEREND:event_procCodeMenuItemActionPerformed
        // TODO add your handling code here:
        if (eiLogin.isUserAdmin(capitalize(PHController.Module) + " Processing Codes"))
        {
    ...
    private void removeTxnFromLog(ISOMsg msg)
    {
        try
        {
            ListModel log = (DefaultListModel) BRMain.bridgeIsoMeter.getLogList() == null ? BRMain.loadBridgeTxnLog() : (DefaultListModel) BRMain.bridgeIsoMeter.getLogList();
    ...
    public void showTxnTree()
    {
        try
        {
            if (BRMain.bridgeIsoMeter != null && !"Y".equalsIgnoreCase(PHController.SuspendPOS))
            {
                txnLog = BRMain.bridgeIsoMeter.getLogList();
            }
            else
            {
                txnLog = BRMain.loadBridgeTxnLog();
            }
            if (txnLog.getSize() <= 0)
            {
                txnLog = BRMain.loadBridgeTxnLog();
            ...
            txnList.add(new EITxn("00", "POS Purchase"));
            txnList.add(new EITxn("01", "Cash Withdrawal"));

            txnList.add(new EITxn("03", "Off-Us Cash Withdrawal"));

            txnList.add(new EITxn("21", "Cash Deposit"));
            txnList.add(new EITxn("31", "Balance Inquiry"));

            txnList.add(new EITxn("38", "Ministatement Inquiry"));
            txnList.add(new EITxn("40", "Funds Transfer"));

            txnList.add(new EITxn("91", "Check Book Request"));

//            txnList.add(new EITxn("80", "School Fee Payment"));
//            txnList.add(new EITxn("81", "School Fee Enquiry"));
//            
//            txnList.add(new EITxn("82", "TV Subscription Payment"));
//            txnList.add(new EITxn("83", "TV Subscription Enquiry"));
//            
//            txnList.add(new EITxn("84", "Tax Payment"));
//            txnList.add(new EITxn("85", "Tax Enquiry"));
            boolean isFirstTxnFound = false;
            for (EITxn eITxn : txnList)
            {
        ...
        catch (Exception ex)
        {
            if (BRMain.bridgeIsoMeter != null && !"Y".equalsIgnoreCase(PHController.SuspendPOS))
            {
                BRMain.bridgeIsoMeter.setLogList(new DefaultListModel());
            }
        }
    }
    ...
    private DefaultMutableTreeNode[] getTxnNodes(EITxn eiTxn)
    {
    ...
    public JTable createMessagesTable(EITxn eiTxn)
    {
    ...
    private void showAboutInfo()
    {
        JOptionPane.showMessageDialog(null,
                "<html><font face=\"Arial\" weight=\"plain\" size=\"2\">PHilae POS Bridge version 2.5.0, Product licensed to FINCA, Lusaka ZAMBIA"
    ...
    public static final javax.swing.JTextPane displayConsole = BRMain.displayArea;
    ...
    private ISOMeter bridgeIsoMeter = new ISOMeter(null, "POS", "SUSPENDED");
